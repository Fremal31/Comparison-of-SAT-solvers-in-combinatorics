...
def generate_plots(results: List[Result], output_dir: str) -> None:
    ...
    # 1. Bar chart — wall-clock time per solver per problem
    try:
        if {'time', 'solver', 'problem'}.issubset(df.columns):
            pivot = df.pivot_table(values='time', index='problem', columns='solver', aggfunc='mean')
            ax = pivot.plot(kind='bar', figsize=(12, 6))
            ax.set_title('Mean Wall-Clock Time per Solver per Problem')
            ax.set_xlabel('Problem')
            ax.set_ylabel('Time (s)')
            ax.legend(title='Solver')
            plt.tight_layout()
            plt.savefig(out / 'time_per_solver_problem.png')
            plt.close()
    except Exception as e:
        print(f"Warning: could not generate time bar chart: {e}")

    # 2. Stacked bar — status counts per solver
    try:
        if {'status', 'solver'}.issubset(df.columns):
            status_counts = df.groupby(['solver', 'status']).size().unstack(fill_value=0)
            ax = status_counts.plot(kind='bar', stacked=True, figsize=(10, 6))
            ax.set_title('Result Status Counts per Solver')
            ax.set_xlabel('Solver')
            ax.set_ylabel('Count')
            ax.legend(title='Status')
            plt.tight_layout()
            plt.savefig(out / 'status_counts_per_solver.png')
            plt.close()
    except Exception as e:
        print(f"Warning: could not generate status bar chart: {e}")
